...
public class ApiCommonValidationPage {
	...
	public boolean isVerifyDrawFreezeDataForLuckyNumber() throws SQLException {
		...
		apiFreezeData = pcposapi.performDrawFreezeForPcPosGame(
				"http://192.168.124.73:8082/DGE_Scheduler/services/drawMgmt/freezeDraw", PCPOSApi.drawid,
				PCPOSApi.gameid);

		con = dbconnection.getDBConnectionDge();
		ResultSet rs = dbconnection.ExecuteQuery(con, LuckyNumberSqlQuery.drawfreezetimeforluckynumber,
				apiFreezeData.get("drawid"));
		while (rs.next()) {
	...
	public boolean isVerifyResultSubmissionDataForLuckyNumber() throws SQLException {
		...
		apiResultSubmissionData = pcposapi.performResultSubmissionForPcPosGame(
				"http://192.168.124.73:8082/DGE_Scheduler/services/drawResultMgmt/performManualWinningEntry",
				PCPOSApi.drawid, PCPOSApi.gameid);
		System.out.println("API Data : " + apiResultSubmissionData);

		if (apiResultSubmissionData.get("responsemsg")
				.equalsIgnoreCase("Perform Manual Winning Entry done successfully:FIRST TIME INSERT")) {
			return true;
		} else {
			return false;
		}
